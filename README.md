...
- `tools/gs3_log_analyze.py` — 웹 뷰어의 "기록 내보내기" + "로그 복사" 결과를 넣으면 wire hex 를 다시 복호화·파싱해 레코드별 전체 필드, 필드별 변동 여부, 판정(혈당 바이트 고정/0/정상, 빠진 index, MTU 잘림)을 출력. `--adversarial` 은 8바이트를 혈당으로 읽는 모든 방법을 대조해 "파서가 틀렸나 센서가 고정인가" 를 판정. `python3 tools/gs3_log_analyze.py 로그.txt --all --adversarial [--csv out.csv]`.
