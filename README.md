...
- `tools/gs3_log_analyze.py` — 웹 뷰어의 "로그 복사" 결과를 넣으면 wire hex 를 다시 복호화·파싱해 레코드별 전체 필드, 필드별 변동 여부, 판정(혈당 바이트 고정/0/정상, 빠진 index, MTU 잘림)을 출력. `python3 tools/gs3_log_analyze.py 로그.txt --all [--csv out.csv]`.
